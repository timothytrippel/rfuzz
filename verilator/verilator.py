...
DefaultFlags = [
    '--assert', '--output-split 0', '-Wno-fatal', '-Wno-WIDTH', '-Wno-STMTDLY',
...
GeneratedFiles = [
    'V{}_classes.mk', 'V{}.cpp', 'V{}.h', 'V{}.mk', 'V{}__Syms.cpp',
    'V{}__Syms.h', 'V{}__ver.d', 'V{}__verFiles.dat', 'V{}__Slow.cpp'
]
...
def check_file_predictions(toplevel, trace, out):
    ...
    produced = {
        ff
        for ff in os.listdir(out) if os.path.isfile(os.path.join(out, ff))
    }
    ...
    if len(not_expected) > 0:
        sys.stderr.write(
            "unexpected files were generated: {}\n".format(not_expected))
        sys.exit(1)
    return produced
...
def verilator_include(includer, inputs, output):
    ...
    r = subprocess.run(' '.join(cmd + args),
                       shell=True,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o',
                        '--output',
                        help='files expected to be generated',
                        nargs='+',
                        required=True)
    parser.add_argument(
        '-i',
        '--input',
        help='verilog file to be simulated, first file is the toplevel',
        nargs='+',
        required=True)
    args = parser.parse_args()

    # extract file basename of toplevel (harness)
    ...
    # extract remaining (verilog) dependencies
    ...
    for dep in dependencies:
        ...
        contains_dep = {
            pp: os.path.isfile(os.path.join(pp, name))
            for pp in include_paths
        }
        assert sum(contains_dep.values()) > 0
        if sum(contains_dep.values()) > 1:
            dirs = ", ".join(k for k, v in contains_dep.items() if v)
            sys.stderr.write(
                "dependency `{}` exists in multiple directories: {}!\n".format(
                    dep, dirs))
            sys.exit(1)
    include_flags = ["-I{}".format(pp) for pp in include_paths]

    # create temporary output directory (for verilator to generated C++ model)
    ...
    # check that Verilator produced the files we expect
    produced = check_file_predictions(toplevel=toplevel,
                                      trace=args.trace,
                                      out=out)

    # print out all C++ files generated by Verilator
    if args.print_out:
        ff = [os.path.join(out, ff) for ff in produced if ff.endswith('.cpp')]
        print('\n'.join(ff))

    # Get location of Verilator src files
